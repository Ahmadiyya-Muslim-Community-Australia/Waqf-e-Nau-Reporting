...
export class NavigateCommand {
  ...
  check(m: Readonly<Model>): boolean {
    return m.isAuthenticated && m.currentPage !== this.target;
  }

  async run(m: Model, page: Page): Promise<void> {
    await page.goto(`${BASE_URL}${this.target}`, {
    ...
    await page.waitForFunction((target) => {
      ...
      return normalize(window.location.pathname) === normalize(target);
    }, this.target);
...
export class VerifyFilteredDataCommand {
  ...
  check(m: Readonly<Model>): boolean {
    return m.isAuthenticated && m.dataVisibility === "filtered";
  }
...
export class VerifyZeroRowsCommand {
  check(m: Readonly<Model>): boolean {
    return m.isAuthenticated && m.dataVisibility === "none";
  }
...
export class VerifyMarkazPlaceholderCommand {
  ...
  async run(m: Model, page: Page): Promise<void> {
    await page
      .locator(".ag-row:not(.ag-header-row)")
      .or(page.getByText("No results", { exact: false }))
      .or(page.getByText("Error", { exact: false }))
      .first()
      .waitFor({ timeout: 30_000 });
