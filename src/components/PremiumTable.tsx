...
function detectColumns(data: unknown): ColDef[] {
    if (!data) return [];
    try {
        const arr = (data as { toArray: () => Record<string, unknown>[] }).toArray();
        if (arr.length === 0) return [];
        ...
        return keys.map((key) => {
            ...
            const isBoolean = typeof val === 'boolean' || val === 0 || val === 1 || val === 'true' || val === 'false';
...
export const PremiumTable: FC<PremiumTableProps> = ({
    ...
    const handleExport = useCallback(() => {
        gridRef.current?.api?.exportDataAsCsv({ fileName: `${queryKey || 'export'}.csv` });
    }, [queryKey]);
        ...
        return (
            <div className="flex h-full items-center justify-center rounded-lg border border-dashed border-red-300 dark:border-red-800">
                <div className="flex flex-col items-center gap-2 text-red-500">
                    ...
                    <p className="max-w-md text-center text-xs text-red-400">{error.message}</p>
                </div>
            </div>
        );
    }
