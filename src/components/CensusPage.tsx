...
import { DynamicFilters, FILTERABLE_MEMBER_COLUMNS, type FilterClause, buildFilterWhere } from './DynamicFilters';
...
export const CensusPage: FC = () => {
    ...
    const [chartFilters, setChartFilters] = useState<CensusFilter[]>(() => getChartFilters());
    const censusReady = useRoomStore((s) => Boolean(s.db.findTableByName?.('census')));
    ...
    const censusWhereFilter = useMemo(() => buildCensusWhere(chartFilters), [chartFilters]);
    const chartFilterModel = useMemo(() => buildCensusAgGridModel(chartFilters), [chartFilters]);
    const chartFilterLabels = useMemo(() => buildCensusFilterLabels(chartFilters), [chartFilters]);
    ...
    const SEARCH_COL_MAP: Record<string, { agGrid: string; sql: string }> = {
        ...
        khidmat_auxiliary: { agGrid: 'Khidmat in Auxiliary', sql: 'c.khidmat_auxiliary' },
        ...
        guidance_huzoor: { agGrid: 'Guidance from Huzoor', sql: 'c.guidance_huzoor' },
        employed: { agGrid: 'Employed', sql: 'c.employed' },
        permission_huzoor: { agGrid: 'Permission from Huzoor', sql: 'c.permission_huzoor' },
        is_moosi: { agGrid: 'Moosi', sql: 'c.is_moosi' },
        syllabus_current: { agGrid: 'Syllabus Up to Date', sql: 'c.syllabus_current' },
    ...
    const searchFilterModel = useMemo(() => {
        ...
        for (const cf of searchColumnFilters) {
            ...
            if (m) model[m.agGrid] = { filterType: 'text', type: 'equals', filter: cf.value };
        }
    ...
    const { columnFilters: memberSearchColumnFilters, freeText: memberFreeText } = useMemo(
        () => parseSearch(debouncedMemberSearch),
    ...
    return (
        <div className="flex h-full flex-col overflow-hidden bg-slate-50 dark:bg-slate-900">
            ...
            >
                ...
                <FilterBadges
                    filters={[
                        ...
                        ...searchColumnFilters.map((cf) => ({ label: cf.column, value: cf.value })),
                    ]}
                />
            </motion.div>
            ...
            >
                ...
                />
                <FilterBadges
                    filters={memberSearchColumnFilters.map((cf) => ({ label: cf.column, value: cf.value }))}
                />
            </motion.div>
